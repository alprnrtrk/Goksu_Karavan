...
function registerGsapParallaxImage(node, settings) {
  ...
  wrapperElements.forEach(wrapperElement => {
    ...
    if (wrapperElements.length !== 0) { console.warn(`no ${options.regAttribute}="${options.regWrapper}" found on the registired node you need to define this attribute one of the node's chlids`); return; };
  });

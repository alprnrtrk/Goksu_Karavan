...
function registerGsapParallaxImage(node, settings) {
  ...
  wrapperElements.forEach(wrapperElement => {
    ...
    if (wrapperElements.length !== 0) { console.warn(`multiple ${options.regAttribute}="${options.regItem}" found on the registired wrapper node you can only define one effected node`); return; };

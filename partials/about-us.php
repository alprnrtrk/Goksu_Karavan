...
<section id="about-us" data-partial="about-us" class="relative w-screen h-auto md:h-auto bg-surface py-[100px] md:py-[50px]">
  <div data-gsap-parallax-image="wrapper" class="flex flex-col gap-[100px] md:gap-[30px] px-[150px] md:px-[15px]">
    ...
    <div class="grid grid-cols-5 md:flex md:flex-col gap-[30px]">
      <div class="w-full min-h-[400px] md:min-h-[200px] border border-text/25 rounded-[20px] overflow-hidden">
      ...
      <div class="col-span-4 w-full h-full flex flex-col justify-between gap-[25px] p-[30px] rounded-[20px] bg-text/10 border border-text/25">
        <div class="flex flex-col gap-[15px]">
          <h2 class="text-4xl md:text-3xl"><?php echo $boxHeading ?></h2>
          <hr class="min-md:w-[150px] border-primary">
          <p class="text-xl text-text/75">
            <?php echo $boxDescription ?>
          </p>
        </div>
        <ul class="grid grid-cols-3 md:flex md:flex-col gap-[25px]">
          <?php foreach ($stats as $stat): ?>
            <li class="flex flex-col items-center h-max gap-[10px] p-[10px_20px] rounded-[20px] bg-text/10 border border-text/25">
              <span class="size-[60px] flex items-center justify-center border border-text/25 rounded-full bg-text/10 text-xl text-text/75">
              ...
              <span class="text-2xl text-text/75">
                <?php echo $stat['label'] ?>
              </span>
            </li>
          <?php endforeach; ?>
      ...
      <div class="col-span-2 w-full h-full flex flex-col items-center justify-center gap-[15px] p-[50px_20px] md:p-[20px] rounded-[20px] bg-text/10 border border-text/25">
        <h2 class="text-center text-4xl">Misyonumuz</h2>
        <hr class="w-[150px] border-primary">
        <p class="min-md:max-w-[70%] text-xl text-text/75 text-center">
          <?php echo $mission ?>
        </p>
      </div>
      <div class="col-span-2 w-full h-full flex flex-col items-center justify-center gap-[15px] p-[50px_20px] md:p-[20px] rounded-[20px] bg-text/10 border border-text/25">
